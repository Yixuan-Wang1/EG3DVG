...
class BaseTrainTester:
    ...
    def get_criterion(args):
        ...
        losses = ['boxes', 'masks', 'labels']#, 'class']
    ...
    def main(self, args):
        ...
        for epoch in range(args.start_epoch, args.max_epoch + 1):
            ...
            # train *
            # print(f"[Rank {dist.get_rank()}] Enter train_one_epoch")
    ...
    @staticmethod
    def _compute_negative_loss(loss, end_points, end_points2):
        prefixes = ['last_'] + [f'{i}head_' for i in range(6 - 1)]
        prefixes = ['proposal_'] + prefixes

        negative_text_feats = end_points2['proj_tokens'].detach()
        text_feats = end_points['proj_tokens']

        negative_align_loss = 0
        for prefix in prefixes:
            indices = end_points[f'{prefix}indices']
            batch_idx = torch.cat([
            ...
            n_logits = (
                    torch.matmul(proj_queries, negative_text_feats.transpose(-1, -2))
                    / 0.07
            )
            logits = (
                    torch.matmul(proj_queries, text_feats.transpose(-1, -2))
                    / 0.07
            )
            ...
            n_modi_positive_map = torch.zeros(n_logits.shape, device=logits.device)
            n_pron_positive_map = torch.zeros(n_logits.shape, device=logits.device)
            n_rel_positive_map = torch.zeros(n_logits.shape, device=logits.device)

            n_pmap_modi = torch.cat([
                t[i] for t, (_, i) in zip(end_points['negative_modify_positive_map'], indices)
            ], dim=0)[..., :n_logits.shape[-1]]
            n_pmap_pron = torch.cat([
                t[i] for t, (_, i) in zip(end_points['negative_pron_positive_map'], indices)
            ], dim=0)[..., :n_logits.shape[-1]]
            n_pmap_rel = torch.cat([
                t[i] for t, (_, i) in zip(end_points['negative_rel_positive_map'], indices)
            ], dim=0)[..., :n_logits.shape[-1]]

            pmap_modi = torch.cat([
                t[i] for t, (_, i) in zip(end_points['modify_positive_map'], indices)
            ], dim=0)[..., :logits.shape[-1]]
            pmap_pron = torch.cat([
                t[i] for t, (_, i) in zip(end_points['pron_positive_map'], indices)
            ], dim=0)[..., :logits.shape[-1]]
            pmap_rel = torch.cat([
                t[i] for t, (_, i) in zip(end_points['rel_positive_map'], indices)
            ], dim=0)[..., :logits.shape[-1]]
            ...
            n_modi_positive_map[idx] = n_pmap_modi
            n_pron_positive_map[idx] = n_pmap_pron
            n_rel_positive_map[idx] = n_pmap_rel

            modi_positive_logits = -logits.masked_fill(~(modi_positive_map > 0), 0).sum(2)
            pron_positive_logits = -logits.masked_fill(~(pron_positive_map > 0), 0).sum(2)
            rel_positive_logits = -logits.masked_fill(~(rel_positive_map > 0), 0).sum(2)
            ...
            n_modi_positive_logits = -n_logits.masked_fill(~(n_modi_positive_map > 0), 0).sum(2)
            n_pron_positive_logits = -n_logits.masked_fill(~(n_pron_positive_map > 0), 0).sum(2)
            n_rel_positive_logits = -n_logits.masked_fill(~(n_rel_positive_map > 0), 0).sum(2)

            n_nb_modi_pos_token = (n_modi_positive_map > 0).sum(2) + 1e-6
            n_nb_pron_pos_token = (n_pron_positive_map > 0).sum(2) + 1e-6
            n_nb_rel_pos_token = (n_rel_positive_map > 0).sum(2) + 1e-6

            boxes_with_pos = (modi_positive_map + pron_positive_map + rel_positive_map).any(2)

            p_logits = torch.exp(
                # positive_logits / nb_pos_token
                + 0.2 * modi_positive_logits / nb_modi_pos_token
                + 0.1 * pron_positive_logits / nb_pron_pos_token
                + 0.2 * rel_positive_logits / nb_rel_pos_token
            ).masked_fill(~boxes_with_pos, 0)
            n_logits = torch.exp(
                # n_positive_logits / n_nb_pos_token
                + 0.2 * n_modi_positive_logits / n_nb_modi_pos_token
                + 0.1 * n_pron_positive_logits / n_nb_pron_pos_token
                + 0.2 * n_rel_positive_logits / n_nb_rel_pos_token
            ).masked_fill(~boxes_with_pos, 0)

            negative_align_loss += -torch.log((p_logits + 1e-9) / (p_logits + 2 * n_logits + 1e-9)).masked_fill(~boxes_with_pos, 0).sum() / num_boxes

        loss += 0.1 * negative_align_loss / (6 + 1)
    ...
    def train_one_epoch(self, epoch, train_loader, model,
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            ...
            inputs2 = self._get_negative_inputs(batch_data)
            ...
            loss, end_points = self._compute_negative_loss(loss, end_points, end_points2)
